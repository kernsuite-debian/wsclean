...
const std::string kAxisNames = "dir,pol";
...
BOOST_FIXTURE_TEST_CASE(set_antennas, H5Fixture) {
  ...
  const std::string kInvalidAnt = "doesnotexist";

  SolTab soltab(group);
  // Table "ant" does not exist yet.
  BOOST_CHECK(!soltab.HasAxis("ant"));
  // Without an "ant" table, GetSubArray works with any antenna name.
  BOOST_CHECK_NO_THROW(
      soltab.GetSubArray("val", kInvalidAnt, 0, 0, 0, 0, 0, 0, 0, 0));

  soltab.SetAntennas(kNames);
  BOOST_CHECK(soltab.HasAxis("ant"));
  // With an "ant" table, GetSubArray only accepts valid antenna names.
  BOOST_CHECK_THROW(
      soltab.GetSubArray("val", kInvalidAnt, 0, 0, 0, 0, 0, 0, 0, 0),
      std::runtime_error);
