...
for script in fetch-latest.sh build-and-sign.sh full-build.sh; do
    if [ -x "debian/$script" ]; then
        echo "  ✓ debian/$script (executable)"
    else
        echo "  ✗ debian/$script (missing or not executable!)"
        exit 1
    fi
done
echo ""
...
if git ls-remote --tags https://gitlab.com/aroffringa/wsclean.git >/dev/null 2>&1; then
    echo "  ✓ Can reach GitLab upstream"
else
...
debian_tools=("debuild" "dch" "debsign")
missing=0
for tool in "${debian_tools[@]}"; do
    if command -v "$tool" &>/dev/null; then
        echo "  ✓ $tool"
    else
        echo "  ✗ $tool (missing - install: sudo apt install devscripts)"
        missing=$((missing+1))
    fi
done
...
if command -v gpg &>/dev/null; then
    ...
    if [ "$KEY_COUNT" -gt 0 ]; then
        echo "  ✓ GPG ($KEY_COUNT signing key(s) available)"
        gpg --list-secret-keys --keyid-format short 2>/dev/null | grep uid | head -1 | sed 's/^/    /'
    else
        echo "No GPG keys found (signing will require manual input)"
    fi
else
...
free_space=$(df /workspaces | awk '{if(NR==2) print int($4)}')
if [ "$free_space" -gt 1000000 ]; then  # > 1GB
    free_gb=$(echo "scale=1; $free_space / 1048576" | bc)
    echo "  ✓ ${free_gb}GB free (sufficient)"
else
    echo "  ✗ Low disk space!"
    exit 1
fi
...
if timeout 5 git clone --depth 1 --branch v3.4 --no-checkout \
    ...
    echo "  ✓ Can clone from upstream"
else
    echo "  ✗ Clone test failed (network issue?)"
    exit 1
fi
echo ""
...
echo "  $ debian/full-build.sh --distro jammy"
echo ""
echo "  Option 2: Fetch only (requires manual build)"
echo "  $ debian/fetch-latest.sh"
echo ""
echo "  Option 3: Build specific version"
echo "  $ debian/fetch-upstream.sh 3.5"
echo "  $ debian/build-and-sign.sh 3.5 jammy"
echo ""
